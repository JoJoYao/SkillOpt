...
def set_optimizer_backend(backend: str) -> None:
    ...
    if OPTIMIZER_BACKEND not in {"openai_chat", "claude_chat", "qwen_chat", "minimax_chat", 'cli_chat'}:
        raise ValueError(
            f"Unsupported optimizer backend: {OPTIMIZER_BACKEND!r}. "
            "Supported values are 'cli_chat', 'openai_chat', 'claude_chat', 'qwen_chat', and 'minimax_chat'."
        )
...
def set_target_backend(backend: str) -> None:
    ...
    if TARGET_BACKEND not in {"openai_chat", "claude_chat", "qwen_chat", "minimax_chat", "codex_exec", "claude_code_exec", 'cli_chat'}:
        raise ValueError(
            f"Unsupported target backend: {TARGET_BACKEND!r}. "
            "Supported values are 'cli_chat', 'openai_chat', 'claude_chat', 'qwen_chat', 'minimax_chat', 'codex_exec', and 'claude_code_exec'."
        )
